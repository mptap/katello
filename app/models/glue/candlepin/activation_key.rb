...
module Glue::Candlepin::ActivationKey
  ...
  module InstanceMethods
    ...
    def get_pools
      pools = Resources::Candlepin::ActivationKey.pools(self.organization.label)
      pools
    end

    def get_keys
      activation_keys = Resources::Candlepin::ActivationKey.get
      activation_keys
    end

    def get_key_pools
      key_pools = Resources::Candlepin::ActivationKey.key_pools(self.cp_id)
      key_pools
    end

    def find(id)
      Resources::Candlepin::ActivationKey.find(id)
    end
    ...
    def find_key_pool(pool_id)
      if get_key_pools.empty?
        return false
      else
        get_key_pools.each do |kp|
          if kp[:id] == pool_id
            return true
          end
        end
